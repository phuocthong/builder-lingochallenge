<template>
  <q-page class="bg-gradient-to-br from-purple-50 to-blue-50">
    <div v-if="!authStore.isLoggedIn" class="flex flex-center" style="height: 70vh">
    ...
    <div v-else class="q-pa-md">
      <div class="row q-gutter-md">
          ...
          <div class="column q-gutter-md">
            ...
            <q-card class="q-pa-lg">
              <div class="text-center">
                ...
                <!-- User Info -->
                <div class="text-h5 text-weight-medium q-mb-xs">{{ authStore.user?.name }}</div>
                <div class="text-grey-6 q-mb-md">@{{ authStore.user?.name.toLowerCase().replace(' ', '') }}</div>

                <!-- Join Date -->
                <div class="row items-center justify-center text-grey-6 text-caption q-mb-lg">
                ...
                <div class="row q-gutter-sm q-mb-lg">
                  <div class="col bg-blue-1 rounded-borders q-pa-sm text-center">
                ...
                <div class="column q-gutter-sm">
                  ...
                  <q-btn color="grey-5" outline icon="lock" label="Đổi mật khẩu" disabled />
                </div>
              </div>
            </q-card>
        ...
        <div class="col-12 col-lg-8">
          <q-card>
            <q-tabs v-model="selectedTab" class="text-grey-6" active-color="primary" indicator-color="primary" align="justify">
            ...
            <q-tab-panels v-model="selectedTab" animated>
              ...
              <q-tab-panel name="profile">
                ...
                <div class="row q-gutter-md q-mb-lg">
                  <div class="col-12 col-sm-6 col-md-3">
                    <q-card class="text-center q-pa-md">
                      <q-icon name="chat" size="md" color="blue" class="q-mb-sm" />
                      <div class="text-h5 text-weight-bold">1,000</div>
                      <div class="text-caption text-grey-6">Tổng câu trả lời</div>
                    </q-card>
                  </div>
                  <div class="col-12 col-sm-6 col-md-3">
                    <q-card class="text-center q-pa-md">
                      <q-icon name="check_circle" size="md" color="green" class="q-mb-sm" />
                      <div class="text-h5 text-weight-bold">82%</div>
                      <div class="text-caption text-grey-6">Tỷ lệ chính xác</div>
                    </q-card>
                  </div>
                  <div class="col-12 col-sm-6 col-md-3">
                    <q-card class="text-center q-pa-md">
                      <q-icon name="book" size="md" color="purple" class="q-mb-sm" />
                  ...
                  <div class="col-12 col-sm-6 col-md-3">
                    <q-card class="text-center q-pa-md">
                      <q-icon name="local_fire_department" size="md" color="orange" class="q-mb-sm" />
                      <div class="text-h5 text-weight-bold">{{ authStore.user?.stats?.streak }}</div>
                ...
                <q-card class="q-pa-lg">
                  ...
                  <div class="bg-gradient-to-br from-purple-100 to-blue-100 rounded-borders q-pa-xl text-center">
                    <q-icon name="bar_chart" size="xl" color="purple" />
                    <div class="text-grey-6 q-mt-sm">Biểu đồ tiến bộ theo tuần</div>
                  </div>
                </q-card>
              </q-tab-panel>
              ...
              <q-tab-panel name="friends">
                <div class="row items-center justify-between q-mb-md">
                  <div class="text-h6">Danh sách bạn bè ({{ friends.length }})</div>
                    ...
                    <q-card>
                      <q-card-section>
                        <div class="row items-center q-mb-md">
                          ...
                          <div class="col">
                            <div class="text-weight-medium">{{ friend.name }}</div>
                        ...
                        <div class="row q-gutter-xs q-mb-md">
                          <div class="col bg-yellow-1 rounded-borders q-pa-xs text-center">
                            <div class="text-caption text-grey-6">Điểm</div>
                            <div class="text-weight-bold text-yellow-8">{{ friend.points.toLocaleString() }}</div>
                          </div>
                          <div class="col bg-red-1 rounded-borders q-pa-xs text-center">
                            <div class="text-caption text-grey-6">Streak</div>
                            <div class="text-weight-bold text-red-8">{{ friend.streak }}</div>
                          </div>
                        </div>

                        <div class="row q-gutter-xs">
                          <q-btn size="sm" outline color="blue" icon="chat" label="Nhắn tin" class="col" @click="sendMessage(friend)" />
                          <q-btn 
                            size="sm" 
                            outline 
                            color="orange" 
              ...
              <q-tab-panel name="tasks">
                <div class="row items-center justify-between q-mb-md">
                  ...
                  <q-chip color="yellow" text-color="dark" icon="monetization_on">
                ...
                <q-tab-panels v-model="taskTab">
                  <q-tab-panel name="daily">
                    <div class="column q-gutter-sm">
                      <q-card v-for="task in dailyTasks" :key="task.id" :class="task.completed ? 'bg-green-1' : ''">
                        <q-card-section>
                          <div class="row items-center">
                            <div class="col">
                              <div class="row items-center q-mb-xs">
                                <q-icon :name="getTaskIcon(task.category)" class="q-mr-sm" />
                                <div class="text-weight-medium" :class="task.completed ? 'text-strike text-grey-6' : ''">
                                  {{ task.title }}
                                </div>
                                <q-icon v-if="task.completed" name="check_circle" color="green" class="q-ml-sm" />
                              </div>
                              <div class="text-caption text-grey-6 q-mb-sm">{{ task.description }}</div>
                              
                              <div class="row items-center q-gutter-md">
                                <div class="col">
                                  <div class="text-caption q-mb-xs">
                                    Tiến độ: {{ task.progress }}/{{ task.target }} ({{ Math.round((task.progress / task.target) * 100) }}%)
                                  </div>
                                  <q-linear-progress 
                                ...
                                <q-chip color="yellow" text-color="dark" size="sm">
                                  +{{ task.points }} điểm
                                </q-chip>
                              </div>
                            </div>
                            ...
                            <div class="col-auto q-ml-md">
                              <div v-if="!task.completed" class="row q-gutter-xs">
                                <q-btn size="sm" outline label="+1" @click="updateTaskProgress(task, task.progress + 1)" />
                                <q-btn size="sm" outline label="Hoàn thành" @click="updateTaskProgress(task, task.target)" />
                              </div>
                              <q-btn size="sm" flat color="red" icon="delete" @click="deleteTask(task.id)" />
                            </div>
                          </div>
                        </q-card-section>
                      </q-card>
                  ...
                  <q-tab-panel name="weekly">
                    <div class="column q-gutter-sm">
                      <q-card v-for="task in weeklyTasks" :key="task.id" :class="task.completed ? 'bg-green-1' : ''">
                        <q-card-section>
                          <div class="row items-center">
                            <div class="col">
                              <div class="row items-center q-mb-xs">
              ...
              <q-tab-panel name="exchange">
                <div class="row items-center justify-between q-mb-md">
                  <div>
                    ...
                    <div class="text-caption text-grey-6">Đổi điểm tích lũy để nhận thẻ cào điện thoại</div>
                  </div>
                  <q-chip color="yellow" text-color="dark" icon="monetization_on">
                    {{ userPoints.toLocaleString() }} điểm
                  </q-chip>
                ...
                <div class="row q-gutter-md">
                  <div v-for="card in phoneCards" :key="card.id" class="col-12 col-sm-6 col-md-4">
                    <q-card :class="card.popular ? 'border-2 border-blue-500' : ''">
                      <q-card-section>
                        <q-chip v-if="card.popular" color="blue" text-color="white" class="q-mb-sm">
                          🔥 Phổ biến
                        </q-chip>

                        <div class="text-center q-mb-md">
                          <q-icon name="phone_android" size="lg" color="grey-6" class="q-mb-sm" />
                          <div class="text-h6 text-weight-bold">{{ card.provider }}</div>
                        ...
                        <div class="q-mb-md">
                          <div class="row items-center justify-between text-caption q-mb-xs">
                            ...
                            <div v-if="card.discount" class="row items-center q-gutter-xs">
                            ...
                            <span v-else>{{ card.cost.toLocaleString() }} điểm</span>
                          </div>
                          
                          <div class="row items-center justify-between">
                            <span class="text-weight-medium">Giá bán:</span>
                            <span class="text-h6 text-yellow-8 text-weight-bold">
                        ...
                        <q-btn 
                          :color="userPoints >= card.cost ? 'primary' : 'grey-5'"
                          :disable="userPoints < card.cost"
                          @click="purchasePhoneCard(card)"
                          class="full-width"
                        >
                          <q-icon :name="userPoints >= card.cost ? 'redeem' : 'block'" class="q-mr-sm" />
...
const dailyTasks = computed(() => tasks.value.filter(task => task.type === 'daily'))
const weeklyTasks = computed(() => tasks.value.filter(task => task.type === 'weekly'))
...
function challengeFriend(friend: Friend) {
  if (!friend.isOnline) {
    Notify.create({
      ...
      caption: 'Bạn có thể gửi lời mời thách đấu và họ sẽ nhận được khi online.',
      position: 'top'
    })
    return
  }
