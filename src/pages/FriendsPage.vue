    ...
    <div v-else class="max-w-6xl mx-auto q-pa-md">
      ...
      <q-card>
        <q-tabs 
          ...
          align="justify"
        >
          <q-tab name="friends" icon="people" :label="`Bạn bè (${friends.length})`" />
          <q-tab name="requests" icon="person_add" :label="`Lời mời (${friendRequests.length})`" />
        ...
        <q-tab-panels v-model="activeTab" animated>
          ...
          <q-tab-panel name="friends">
            <div v-if="friends.length === 0" class="text-center q-py-xl">
              <q-icon name="favorite_border" size="4rem" color="grey-4" class="q-mb-md" />
              <h3 class="text-h6 text-grey-6 q-mb-sm">Chưa có bạn bè</h3>
              <p class="text-grey-5 q-mb-lg">Hãy tìm kiếm và kết bạn với người khác!</p>
              <q-btn color="primary" icon="search" label="Tìm bạn bè" @click="activeTab = 'search'" />
            </div>

            <div v-else class="row q-gutter-md">
              <div class="col-12 col-md-6 col-lg-4" v-for="friend in friends" :key="friend.id">
                <q-card class="card-hover">
                  <q-card-section>
                    <!-- Avatar and Status -->
                    <div class="row items-center q-gutter-md q-mb-md">
                      <q-avatar size="48px">
                        <div class="bg-gradient-to-r from-purple-500 to-blue-500 text-white font-bold rounded-full w-full h-full flex items-center justify-center">
                          {{ friend.avatar }}
                        </div>
                        <q-badge floating :color="friend.isOnline ? 'green' : 'grey'" rounded />
                      </q-avatar>
                      
                      <div class="col">
                        <h3 class="text-subtitle1 text-weight-medium q-ma-none">{{ friend.name }}</h3>
                        <p class="text-caption text-grey-6 q-ma-none">{{ friend.username }}</p>
                        <p class="text-caption" :class="friend.isOnline ? 'text-green-6' : 'text-grey-5'">
                    ...
                    <div class="row q-gutter-xs q-mb-md">
                      <div class="col-6 bg-yellow-1 q-pa-sm rounded-borders text-center">
                        <q-icon name="emoji_events" color="yellow-7" size="sm" class="q-mb-xs" />
                        <div class="text-caption text-weight-medium">#{{ friend.stats.rank }}</div>
                      </div>
                      <div class="col-6 bg-green-1 q-pa-sm rounded-borders text-center">
                        <q-icon name="star" color="green-7" size="sm" class="q-mb-xs" />
                        <div class="text-caption text-weight-medium">{{ friend.stats.totalCorrect }}</div>
                      </div>
                    </div>
          ...
          <q-tab-panel name="requests">
            <div v-if="friendRequests.length === 0" class="text-center q-py-xl">
              <q-icon name="person_add_alt" size="4rem" color="grey-4" class="q-mb-md" />
            ...
            <div v-else class="q-gutter-md">
              <q-card v-for="request in friendRequests" :key="request.id" bordered>
                <q-card-section>
                  <div class="row items-center justify-between">
                    <div class="col row items-center q-gutter-md">
                      ...
                      <div>
                        <h3 class="text-subtitle1 text-weight-medium q-ma-none">{{ request.name }}</h3>
                        <p class="text-caption text-grey-6 q-ma-none">{{ request.username }}</p>
          ...
          <q-tab-panel name="search">
            ...
            <div v-if="searchResults.length > 0" class="q-mb-lg">
              ...
              <div class="q-gutter-md">
                <q-card v-for="person in searchResults" :key="person.id" bordered>
                  <q-card-section>
                    <div class="row items-center justify-between">
                      <div class="col row items-center q-gutter-md">
                        <q-avatar size="48px">
                          ...
                          <q-badge floating :color="person.isOnline ? 'green' : 'grey'" rounded />
                        </q-avatar>
                        
                        <div>
                          <h3 class="text-subtitle1 text-weight-medium q-ma-none">{{ person.name }}</h3>
                          <p class="text-caption text-grey-6 q-ma-none">{{ person.username }}</p>
                          <div class="text-caption text-grey-5">
                            🏆 Hạng #{{ person.stats.rank }} • ✅ {{ person.stats.totalCorrect }} đúng • 🔥 {{ person.stats.accuracy }}% chính xác
                          </div>
                        </div>
                      </div>
            ...
            <!-- Quick Suggestions -->
            <div>
              <h3 class="text-h6 text-weight-medium q-mb-md">💡 Gợi ý kết bạn</h3>
              <div class="row q-gutter-md">
                <div class="col-12 col-md-6">
                  <div class="q-pa-md bg-gradient-to-r from-purple-50 to-blue-50 rounded-borders">
                    <div class="row items-center q-gutter-md q-mb-md">
                      <q-icon name="emoji_events" size="2rem" color="yellow-7" />
                      <div>
                        <h4 class="text-subtitle1 text-weight-medium q-ma-none">Kết bạn với Top Players</h4>
                        <p class="text-caption text-grey-6 q-ma-none">Học hỏi từ những người giỏi nhất</p>
                      </div>
                    </div>
                    <q-btn outline color="primary" label="Xem danh sách" class="full-width" dense />
                  </div>
                </div>

                <div class="col-12 col-md-6">
                  <div class="q-pa-md bg-gradient-to-r from-green-50 to-blue-50 rounded-borders">
                    <div class="row items-center q-gutter-md q-mb-md">
                      <q-icon name="email" size="2rem" color="green-7" />
                      <div>
                        <h4 class="text-subtitle1 text-weight-medium q-ma-none">Mời qua Email</h4>
                        <p class="text-caption text-grey-6 q-ma-none">Mời bạn bè tham gia EnglishBot</p>
                      </div>
                    </div>
                    <q-btn outline color="green" label="Gửi lời mời" class="full-width" dense />
                  </div>
                </div>
              </div>
            </div>
...
const sendFriendRequest = (targetUser: Friend) => {
  ...
  searchResults.value = searchResults.value.filter(p => p.id !== targetUser.id)
}
